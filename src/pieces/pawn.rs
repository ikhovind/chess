...
pub fn possible_p(b: &Board, last_move: Move, white: u8) -> Vec<Move> {
    let mut list: Vec<Move> = Vec::new();
    let opposing_pieces = if white == 1 { b.black_pieces } else { b.white_pieces };
    let mut pawn_moves = (b.pieces[(P_INDEX + white) as usize] << 9) & (opposing_pieces) & (!RANK_MASKS[7]) & (!FILE_MASKS[0]); // capture right
    ...
    pawn_moves = (b.pieces[(P_INDEX + white) as usize] << 7) & (opposing_pieces) & (!RANK_MASKS[7]) & (!FILE_MASKS[7]); // capture left
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
            list.push(Move::new_move(i-7,i, true));
        }
    }
    pawn_moves=(b.pieces[(P_INDEX + white) as usize] << 8)&b.empty&!RANK_MASKS[7];//move 1 forward
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
            list.push(Move::new_move(i-8,i, false));
        }
    }
    pawn_moves=((b.pieces[(P_INDEX + white) as usize] << 16) & (b.empty & (b.empty << 8))) & RANK_MASKS[3];//move 2 forward
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
    ...
    pawn_moves=(b.pieces[(P_INDEX + white) as usize] << 7)&opposing_pieces&RANK_MASKS[7]&!FILE_MASKS[0];//pawn promotion by capture left
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
    ...
    pawn_moves=(b.pieces[(P_INDEX + white) as usize] << 9)&opposing_pieces&RANK_MASKS[7]&!FILE_MASKS[7];//pawn promotion by capture right
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
    ...
    pawn_moves=(b.pieces[(P_INDEX + white) as usize] << 8)&b.empty&RANK_MASKS[7];//pawn promotion by move 1 forward
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
    ...
    // en passant
    pawn_moves = ((b.pieces[(P_INDEX + white) as usize] << 9) & (opposing_pieces << 8) & (!RANK_MASKS[7]) & (!FILE_MASKS[7])) & if Move::last_move_was_double_push(last_move) { 2_u64.pow(last_move.from as u32) << 8} else { 0 };  // capture right
    for i in 0..64 {
        if ((pawn_moves >> i) & 1) == 1 {
    ...
    pawn_moves = ((b.pieces[(P_INDEX + white) as usize] << 7) & (opposing_pieces << 8) & (!RANK_MASKS[7]) & (!FILE_MASKS[0])) & if Move::last_move_was_double_push(last_move) { 2_u64.pow(last_move.to as u32) << 8} else { 0 }; // capture left
    for i in 0..64 {
        if ((pawn_moves>>i)&1)==1 {
