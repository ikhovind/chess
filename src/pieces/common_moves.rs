...
pub fn h_and_vmoves(s: u8, opposing_non_k: u64, own_pieces: u64) -> u64 {
    ...
    return !own_pieces & ((possibilities_horizontal & RANK_MASKS[(s as usize) / 8]) | (possibilities_vertical & FILE_MASKS[(s as usize) % 8]));
}
...
pub fn d_and_anti_d_moves(s: u8, opposing_non_k: u64, own_pieces: u64) -> u64 {
    ...
    return !own_pieces & ((possibilities_diagonal &DIAGONAL_MASKS[((s as usize) / 8) + ((s as usize) % 8)]) | (possibilities_anti_diagonal &ANTI_DIAGONAL_MASKS[((s as usize) / 8) + 7 - ((s as usize) % 8)]));
}
