...
pub fn possible_p(b: &Board, captures: bool) -> Vec<Move> {
    let index = if b.white_turn { 1 } else { 0 };
    let mut list: Vec<Move> = Vec::with_capacity((b.pieces[P_INDEX + index].count_ones()) as usize);
    ...
    if b.white_turn {
        ...
        if Move::last_move_was_double_push(b.last_move) {
            ...
            for i in (pawn_moves.trailing_zeros() as u8)..(64u8 - pawn_moves.leading_zeros() as u8) {
                if pawn_moves & b.get_pinned_slide(i - 9) & (1u64 << i) != 0 && check_ep_legal(b, 1 << (i - 9), 1 << (i - 8)) {
            ...
            for i in (pawn_moves.trailing_zeros() as u8)..(64u8 - pawn_moves.leading_zeros() as u8) {
                if pawn_moves & b.get_pinned_slide(i - 7) & (1u64 << i) != 0 && check_ep_legal(b, 1 << (i - 7), 1 << (i - 8)) {
...
fn check_ep_legal(b: &Board, move_piece: u64, taken_piece: u64) -> bool {
    let index = if b.white_turn { 1 } else { 0 };
    if b.pieces[(K_INDEX + index) as usize] == 0 { return true; };
    let opp = if b.white_turn { b.get_black_pieces() - taken_piece } else { b.get_white_pieces() - taken_piece };
    let own = if b.white_turn { b.get_white_pieces() - move_piece } else { b.get_black_pieces() - move_piece };
