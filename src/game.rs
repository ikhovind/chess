...
impl Board {
    ...
    pub fn possible_pw(&mut self, last_double_pawn: u64) -> String {
        ...
        pawn_moves = (self.pawns[1] << 7) & (self.black_pieces) & (!RANK_8) & (!FILE_H); // capture right
        for i in 0..64 {
            if ((pawn_moves>>i)&1)==1 {
        ...
        pawn_moves=(self.pawns[1] << 7)&self.black_pieces&RANK_8&!FILE_A;//pawn promotion by capture left
        for i in 0..64 {
            if ((pawn_moves>>i)&1)==1 {
                list.push_str(&*format!("{}{}{}{}{}{}{}{}", (i % 8 - 1) + (i % 8), "QP", (i % 8 - 1) + (i % 8), "RP", (i % 8 - 1) + (i % 8), "BP", (i % 8 - 1) + (i % 8), "NP"));
            }
        }
        pawn_moves=(self.pawns[1] << 9)&self.black_pieces&RANK_8&!FILE_H;//pawn promotion by capture right
        for i in 0..64 {
        ...
        pawn_moves = ((self.pawns[1] << 9) & (self.black_pieces << 8) & (!RANK_8) & (!FILE_H)) & last_double_pawn;  // capture right
        for i in 0..64 {
            if ((pawn_moves >> i) & 1) == 1 {
                list.push_str(&*((i / 8) + (i % 8) + (i / 8 + 1) + (i % 8 + 1)).to_string());
            }
        ...
        pawn_moves = ((self.pawns[1] << 7) & (self.black_pieces << 8) & (!RANK_8) & (!FILE_A)) & last_double_pawn; // capture left
        for i in 0..64 {
            if ((pawn_moves>>i)&1)==1 {
                print!("{} {} {} {}", (i / 8), (i % 8 + 2), (i / 8 + 1), (i % 8 + 1));
                list.push_str(&*((i/8)+(i%8 + 1)+(i/8)+(i%8)).to_string());
